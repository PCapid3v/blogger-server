...
export async function validateReaderRole(req, res, next) {
  if (!req.user) {
    return sendMessageResponse(res, 500, "Unable to verify user");
  }
...
export async function validateAuthentication(req, res, next) {
  ...
  if (decodedToken.userRole === "ADMIN") {
    const foundAdmin = await Admin.findByUserId(decodedToken.userId);
    req.Admin = foundAdmin;
  }
  if (decodedToken.userRole === "READER") {
    const foundReader = await Reader.findByUserId(decodedToken.userId);
    req.Reader = foundReader;
  }
